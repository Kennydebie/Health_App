...
export function ProgressRing({ value, max, size = 184, label, valueLabel, warning = false }: ProgressRingProps) {
  const radius = 47;
  const circumference = 2 * Math.PI * radius;
  const percent = Math.min(Math.max(value / Math.max(max, 1), 0), 1);
  return (
    <div className={`progress-ring ${warning ? 'progress-ring--warning' : ''}`} style={{ width: size, height: size }}>
