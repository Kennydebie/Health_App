...
import { adherenceSuggestion, analyzeWeeklyProgram, exceedsBalanceLevel, isEquipmentCompatible, progressionRecommendation, squatProgressionLevels, squatReadinessCriteria } from '../lib/workout';
...
function WeeklyOverview({ controller }: { controller: AppController }) {
  ...
  const week = getWeekSnapshot(controller.data);
  const adherence = adherenceSuggestion(controller.data.program, controller.data.sessions);
  const cardioPercent = Math.min(100, week.cardioMinutes / controller.data.weeklyCardioTarget * 100);
  ...
  return <>
    <section className="weekly-overview card">
      <header><div><p className="eyebrow">This week</p><h2>Actual progress</h2><p>Completed sessions and logged cardio only. Planned volume is shown separately below.</p></div><div className="weekly-score"><span><Dumbbell size={17} /><strong>{week.completedStrength}/{week.plannedStrength}</strong> strength</span><span><Activity size={17} /><strong>{week.cardioMinutes}/{controller.data.weeklyCardioTarget}</strong> cardio min</span></div></header>
...
export function WorkoutPage({ controller, activeSessionId, setActiveSessionId, onStartWorkout }: WorkoutPageProps) {
  ...
  return <div className="page workout-page">
    ...
    {view === 'program' ? <>
      ...
      <section className="day-tabs premium-day-tabs" aria-label="Weekly workout schedule">{controller.data.program.map((item, index) => { const planDay = week.days[index]; const completed = planDay.status === 'completed'; const isSelected = day.id === item.id; const isToday = planDay.date === today; const status = ({ completed: 'Completed', today: 'Today', upcoming: 'Upcoming', skipped: 'Skipped', missed: 'Missed', 'not-scheduled': 'Not scheduled' } as const)[planDay.status]; const DayIcon = item.isRestDay ? isToday ? HeartPulse : BedDouble : Dumbbell; return <button type="button" key={item.id} aria-current={isSelected ? 'date' : undefined} aria-label={`${item.label}, ${displayWorkoutTitle(item)}, ${status}`} className={`${isSelected ? 'active' : ''} ${item.isRestDay ? 'rest-day' : 'training-day'} ${completed ? 'completed' : ''} ${isToday ? 'current-day' : ''} ${planDay.status === 'missed' ? 'past-day' : ''}`} onClick={() => setSelectedDayId(item.id)}><div className="day-tab-top"><span>{item.label}</span><i><DayIcon size={16} /></i></div><strong>{displayWorkoutTitle(item)}</strong><small>{completed ? <BadgeCheck size={14} /> : isToday ? <Circle size={13} /> : null}{status}</small></button>; })}</section>
