...
function ExerciseDetail({ exercise, onClose }: { exercise: Exercise; onClose: () => void }) {
  const map = exercise.muscleMap;
  const video = exercise.video;
  return <Modal open onClose={onClose} title={exercise.name} subtitle="Muscles, technique and safety" size="large">
    <div className="exercise-detail">
      <div className="exercise-detail-hero"><div>{video.status === 'verified' ? <div className="demo-panel">
          <iframe src={video.embedUrl} title={`${exercise.name} technique demonstration by ${video.sourceName}`} loading="lazy" referrerPolicy="strict-origin-when-cross-origin" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen />
          <footer><span><Play size={16} /><span><strong>{video.title}</strong><small>Reviewed {video.lastReviewed} · {video.sourceName}</small></span></span><a href={video.sourceUrl} target="_blank" rel="noreferrer">Open source <ExternalLink size={15} /></a></footer>
        </div> : <div className="video-fallback compact"><Info size={19} /><div><strong>Video not available for this exact variation</strong><p>{video.reason} Use the written setup and execution cues below.</p></div></div>}</div><aside className="exercise-anatomy-detail"><div><p className="eyebrow">Anatomical muscle map</p><h3>{map.preferredView === 'both' ? 'Front and back' : map.preferredView === 'front' ? 'Front view' : 'Back view'}</h3></div><AnatomicalMuscleMap map={map} view={map.preferredView} size="detail" interactive /><MuscleMapLegend /></aside></div>
      ...
      <div className="muscle-detail-copy"><div><span>Primary</span><strong>{muscleList(map.primary)}</strong></div><div><span>Secondary</span><strong>{muscleList(map.secondary)}</strong></div>{map.stabilizers?.length ? <div><span>Stabilizers</span><strong>{muscleList(map.stabilizers)}</strong></div> : null}<p>The primary muscles do most of the programmed work. Secondary muscles assist the movement, while stabilizers help maintain position and control.</p></div>
...
function SupportingSection({ storageId, title, summary, defaultOpen = false, children }: { storageId: string; title: string; summary: string; defaultOpen?: boolean; children: ReactNode }) {
  const storageKey = `project75:workout-section:${storageId}`;
  ...
  useEffect(() => {
    try { localStorage.setItem(storageKey, String(open)); } catch { /* UI preference only; safe to ignore unavailable storage. */ }
  }, [open, storageKey]);
  return <section className={`workout-support-section ${open ? 'open' : ''}`}><button type="button" className="workout-support-toggle" aria-expanded={open} onClick={() => setOpen((value) => !value)}><span><strong>{title}</strong><small>{summary}</small></span><ChevronRight size={18}/></button>{open ? <div className="workout-support-content">{children}</div> : null}</section>;
}

function SessionMuscleOverview({ day }: { day: WorkoutDay }) {
  const exposure = useMemo(() => combinedMuscleExposure(day.exercises, exerciseMuscleMap), [day.exercises]);
  const map = useMemo(() => exposureMuscleMap(exposure), [exposure]);
  const details = useMemo(() => Object.fromEntries(exposure.map((item) => [item.muscle, `${item.exerciseCount} ${item.exerciseCount === 1 ? 'exercise' : 'exercises'} · ${item.workingSets} programmed sets · ${item.primarySets} primary · ${item.secondarySets} secondary`])), [exposure]);
  return <SupportingSection storageId="session-anatomy" title="Workout anatomy" summary={`${exposure.length} muscles mapped · open front and back view`}><section className="session-muscle-overview" aria-labelledby="session-muscle-title"><header><div><p className="eyebrow">Workout anatomy</p><h3 id="session-muscle-title">Muscles in this workout</h3><p>Highlight strength reflects combined programmed exposure—not precise muscle activation.</p></div><MuscleMapLegend presentation="session-exposure" /></header><div className="session-muscle-overview__body"><AnatomicalMuscleMap map={map} view="both" size="session" interactive details={details}/><div className="session-muscle-stats">{exposure.slice(0, 10).map((item) => <span key={item.muscle}><strong>{MUSCLE_LABELS[item.muscle]}</strong><small>{item.exerciseCount} {item.exerciseCount === 1 ? 'exercise' : 'exercises'} · {item.workingSets} working sets</small><em>{item.primarySets} primary · {item.secondarySets} secondary</em></span>)}</div></div></section></SupportingSection>;
}
...
function ProgramEditor({ day, program, profile, onSave, onClose }: { day: WorkoutDay; program: WorkoutDay[]; profile: UserProfile; onSave: (day: WorkoutDay) => void; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title={`Edit ${displayWorkoutTitle(day)}`} subtitle="Review any warnings before saving your changes." size="large" footer={<><button type="button" className="text-button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit" form="program-editor-form"><Save size={18} /> Save workout</button></>}>
    <form id="program-editor-form" className="program-editor" onSubmit={(event) => { event.preventDefault(); onSave(draft); onClose(); }}>
      ...
      {analyzedProgram.warnings.length ? <div className="program-warnings"><header><AlertTriangle size={17} /><strong>Workout checks</strong><span>Review recommended</span></header>{analyzedProgram.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div> : <div className="program-clear"><CheckCircle2 size={17} /> No equipment, balance or muscle-coverage warnings.</div>}
    </form>
  </Modal>;
}
...
function ActiveWorkout({ controller, session, onBack, onFinished }: { controller: AppController; session: WorkoutSession; onBack: () => void; onFinished: () => void }) {
  ...
  if (!currentExercise) return <div className="empty-state card"><Info size={30} /><h3>This saved workout cannot be resumed</h3><p>Your completed history is still available. Return to the plan and start a current workout.</p><button type="button" className="secondary-button" onClick={onBack}>Back to workout plan</button></div>;
...
function TrainingCalendar({ sessions, onOpen }: { sessions: WorkoutSession[]; onOpen: (session: WorkoutSession) => void }) {
  ...
  return <section className="training-calendar card">
    <header><div><p className="eyebrow">Training calendar</p><h2>{monthLabel}</h2><p>Markers show workouts completed on each date.</p></div><div className="calendar-month-controls"><button type="button" className="icon-button" onClick={() => shiftMonth(-1)} aria-label="Previous month"><ChevronLeft size={18}/></button><button type="button" className="calendar-today-button" onClick={() => setMonthKey(today.slice(0, 7))}>Today</button><button type="button" className="icon-button" onClick={() => shiftMonth(1)} aria-label="Next month"><ChevronRight size={18}/></button></div></header>
...
function WorkoutHistory({ sessions, onOpen }: { sessions: WorkoutSession[]; onOpen: (session: WorkoutSession) => void }) {
  ...
  return <section className="history-log"><div className="section-title"><div><h2>Workout history</h2></div><span>{completed.length} saved</span></div><div className="history-list">{completed.length ? completed.map((session) => <button type="button" className="history-card card" key={session.id} onClick={() => onOpen(session)}><div className="history-date"><span>{new Date(`${session.date}T12:00:00`).getDate()}</span><small>{new Intl.DateTimeFormat('en', { month: 'short' }).format(new Date(`${session.date}T12:00:00`))}</small></div><div><p className="eyebrow">{prettyDate(session.date)}</p><h3>{displayWorkoutTitle({ id: session.dayId, workoutId: session.workoutId, title: session.title })}</h3><span>{formatDuration(session.durationSeconds)} · {session.sets.filter((set) => set.completed && !set.isWarmup).length} working sets</span></div><div className="volume-stat"><strong>{Math.round(workoutVolume(session)).toLocaleString()}</strong><span>kg volume</span></div><ChevronRight size={19} /></button>) : <div className="empty-state card"><History size={30} /><h3>No workouts yet</h3><p>Finish your first workout to add it to the calendar and history.</p></div>}</div></section>;
}
...
function WeeklyOverview({ controller }: { controller: AppController }) {
  ...
  return <>
    <SupportingSection storageId="weekly-targets" title="Weekly targets and workout balance" summary={`${balance.strength}/${balance.targets.strengthSessions} strength workouts · ${balance.cardioMinutes}/${balance.cardioTarget} cardio min`}><section className="weekly-overview card">
      <header><div><h2>Weekly targets</h2><p>Completed workouts count toward these targets. Skipped or missed workouts are not carried forward.</p></div></header>
      <div className="weekly-balance-summary"><span><strong>{balance.upper}/{balance.targets.upperSessions}</strong><small>Upper-body workouts</small></span><span><strong>{balance.lower}/{balance.targets.lowerSessions}</strong><small>Leg workouts</small></span><span><strong>{balance.strength}/{balance.targets.strengthSessions}</strong><small>Strength workouts</small></span><span><strong>{balance.cardioMinutes}/{balance.cardioTarget}</strong><small>Cardio minutes</small></span><span><strong>{balance.recoveryOpportunities}</strong><small>Recovery days</small></span></div>
      ...
      <div className="muscle-coverage-summary"><p><strong>Trained:</strong> {balance.trainedGroups.length ? balance.trainedGroups.join(' · ') : 'No completed working sets yet'}</p><p><strong>Not yet trained:</strong> {balance.untrainedGroups.join(' · ') || 'All tracked muscle groups trained'}</p></div>
      <details className="program-balance" open={balanceOpen} onToggle={(event) => setBalanceOpen(event.currentTarget.open)}>
        <summary><span><strong>Workout balance</strong><small>Planned movement patterns and direct working sets</small></span><ChevronRight size={18} /></summary>
        ...
        {analysis.warnings.length ? <div className="overview-warnings"><AlertTriangle size={17} /><div><strong>Workout balance warnings</strong>{analysis.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div></div> : null}
    ...
    <Modal open={Boolean(templateToApply)} onClose={() => setTemplateToApply(null)} title="Apply a different training template?" subtitle="Food logs, weight history, completed workouts, and recent bests remain unchanged." size="small">
...
function SessionChooser({ controller, date, plan, onClose, onSelected }: { controller: AppController; date: string; plan: TrainingDayPlan; onClose: () => void; onSelected: () => void }) {
  ...
  return <Modal open onClose={onClose} title="Choose workout" subtitle={`${prettyDate(date, true)} · Follow the recommendation or select another workout.`} size="large">
    <div className="session-chooser">
      <section className="recommended-session-panel"><div><p className="eyebrow">Recommended workout</p><h2>{recommendation.name}</h2><p>{plan.recommendationReason}</p></div><dl><div><dt>Duration</dt><dd>{recommendation.duration}</dd></div><div><dt>Main muscles</dt><dd>{recommendation.muscleGroups.join(' · ') || 'Full-body recovery'}</dd></div><div><dt>Recovery</dt><dd>{recoveryForSession(controller.data, recommendation.id, date).indicator}</dd></div><div><dt>Weekly target</dt><dd>{supportsTarget(recommendation.id) ? 'Supports a remaining target' : 'Adds a recovery day'}</dd></div></dl><button type="button" className="primary-button" onClick={() => { controller.selectTrainingSession(date, recommendation.id, 'adaptive_recommendation'); onSelected(); }}><Check size={17} /> Select recommended workout</button></section>
      <div className="chooser-heading"><div><h3>All workouts</h3></div><span>Select any available option</span></div>
      <div className="session-option-grid">{options.map((option) => {
        ...
        return <button type="button" className={`session-option ${selected ? 'selected' : ''} recovery-${recovery.indicator.toLowerCase().replace(/\s+/g, '-')}`} key={option.id} onClick={() => choose(option.id)}><header><span>{option.name}</span>{selected ? <Check size={15} /> : null}</header><p>{option.muscleGroups.join(' · ') || (option.type === 'cardio' ? 'Cardio and recovery' : 'Full-body recovery')}</p><dl><div><dt>Time</dt><dd>{option.duration}</dd></div><div><dt>Recovery</dt><dd>{recovery.indicator}</dd></div><div><dt>This week</dt><dd>{performed} completed</dd></div><div><dt>Last done</dt><dd>{last ? prettyDate(last) : 'No history'}</dd></div></dl><footer>{supportsTarget(option.id) ? <><Target size={13} /> Supports a remaining weekly target</> : <><HeartPulse size={13} /> Optional workout</>}</footer></button>;
      })}</div>
...
function WeekCustomizer({ controller, referenceDate, onClose }: { controller: AppController; referenceDate: string; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title="Customize this week" subtitle="Move, swap, or restore sessions. Completed history is never changed." size="large" footer={<div className="customizer-footer"><button type="button" className="secondary-button" onClick={() => controller.recalculateTrainingPlan(referenceDate)}><RefreshCw size={17}/> Recalculate remaining week</button><button type="button" className="secondary-button" onClick={() => controller.restoreRecommendedWeek(referenceDate)}><RefreshCw size={17}/> Restore recommendations</button><button type="button" className="text-button" onClick={onClose}>Done</button></div>}>
    <div className="week-customizer">
      ...
      {preview ? <div className="week-move-preview"><p className="eyebrow">Review change</p><h3>{sessionOption(controller.data.program, fromPlan!.selectedSessionTemplateId).name} → {prettyDate(preview.to)}</h3>{previewWarnings.length ? previewWarnings.map((warning) => <div className={`planner-warning ${warning.tone}`} key={warning.code}><AlertTriangle size={17}/><p><strong>{warning.title}</strong>{warning.message}</p></div>) : <p className="preview-clear"><CheckCircle2 size={17}/> No recent-training conflict was found. Recovery still depends on how you feel that day.</p>}<div><button type="button" className="primary-button" onClick={() => { controller.moveTrainingSession(preview.from, preview.to, preview.swap); setPreview(null); }}><Check size={17}/> Apply {preview.swap ? 'swap' : 'move'}</button><button type="button" className="text-button" onClick={() => setPreview(null)}>Cancel</button></div></div> : null}
    </div>
  </Modal>;
}
...
function SquatProgressionCard({ controller, onDetails }: { controller: AppController; onDetails: (exercise: Exercise) => void }) {
  ...
  return <SupportingSection storageId="squat-progression" title="Squat progression" summary={`Level ${currentIndex + 1}/${squatProgressionLevels.length} · ${current.label}`}><article className="card squat-progression-card"><div className="section-title"><div><h2>Squat progression</h2></div><span>Level {currentIndex + 1}/{squatProgressionLevels.length}</span></div><p>Complete every readiness item before moving to a harder variation.</p><button type="button" className="current-variation" onClick={() => onDetails(exerciseMap.get(current.id)!)}><span><Footprints size={18} /></span><div><strong>{current.label}</strong><small>Current variation · View technique and safety guidance</small></div><ChevronRight size={18} /></button><div className="readiness-list">{squatReadinessCriteria.map((criterion, index) => <label key={criterion}><input type="checkbox" checked={checked[index]} onChange={() => setChecked((values) => values.map((value, itemIndex) => itemIndex === index ? !value : value))} /><span>{checked[index] ? <Check size={14} /> : null}</span>{criterion}</label>)}</div><div className="progression-actions">{previous ? <button type="button" className="secondary-button" onClick={() => { controller.setSquatProgression(previous.id); setChecked(squatReadinessCriteria.map(() => false)); }}><Minus size={16} /> Regress to {previous.label}</button> : <span>This is the first supported level.</span>}{next ? <button type="button" className="primary-button" disabled={!ready} onClick={() => { controller.setSquatProgression(next.id); setChecked(squatReadinessCriteria.map(() => false)); }}><Check size={16} /> Advance to {next.label}</button> : <span>This is the final progression level.</span>}</div></article></SupportingSection>;
}

function WorkoutGuidance({ restDay }: { restDay: boolean }) {
  return <SupportingSection storageId="training-guidance" title={restDay ? 'Recovery guidance' : 'Progression and warm-up guidance'} summary={restDay ? 'Cardio target and fatigue check' : 'How to progress safely and use warm-up sets'}><aside className="workout-guidance">{restDay ? <><article className="card overload-card"><span className="metric-icon blue"><Activity size={19} /></span><h3>Weekly cardio target</h3><p>Add minutes gradually with brisk walking, cycling or another low-impact activity.</p><div><Sparkles size={16} /> Short sessions count toward the target</div></article><article className="card squat-card"><span className="metric-icon lime"><BedDouble size={19} /></span><h3>Fatigue check</h3><p>Skip hard intervals when your legs are still fatigued from strength training.</p></article></> : <><article className="card overload-card"><span className="metric-icon lime"><BarChart3 size={19} /></span><h3>Progression</h3><p>Increase repetitions within the target range first. Add weight after reaching the top of the range with the planned RIR in two sessions.</p><div><Sparkles size={16} /> Training to failure is not required</div></article><article className="card warmup-card"><span className="metric-icon orange"><ListChecks size={19} /></span><h3>Warm-up sets</h3><p>Warm-up sets prepare you for the exercise and do not count toward working-set volume.</p></article></>}</aside></SupportingSection>;
}
...
export function WorkoutPage({ controller, activeSessionId, setActiveSessionId, onStartWorkout }: WorkoutPageProps) {
  ...
  const [startIntent, setStartIntent] = useState<{ templateId: SessionTemplateId; date: string; mode: 'full' | 'short' } | null>(null);
  const today = toDateKey();
  ...
  const selectedWeekday = weekdayFormatter.format(new Date(`${selectedSnapshot.date}T12:00:00`));
  ...
  const selectedSourceLabel = selectedPlan ? selectionSourceLabel(selectedPlan.selectionSource) : 'Original plan';
  ...
  const launchStart = (templateId: SessionTemplateId, date: string, mode: 'full' | 'short') => {
    if (mode === 'full') onStartWorkout(templateId, date);
    else {
      const sessionId = controller.startShortWorkoutTemplate(templateId, date);
      if (sessionId) { setActiveSessionId(sessionId); setHideActiveSession(false); }
    }
  };
  const requestStart = (templateId: SessionTemplateId, date: string, mode: 'full' | 'short' = 'full') => {
    const warnings = plannerWarnings(controller.data, date, templateId, selectedPlan?.readinessResponse);
    if (warnings.length) setStartIntent({ templateId, date, mode });
    else launchStart(templateId, date, mode);
  };
  const continueStart = () => {
    ...
    launchStart(startIntent.templateId, startIntent.date, startIntent.mode);
    setStartIntent(null);
  };
  ...
  return <div className="page workout-page">
    <header className="page-header workout-header"><div><h1>Workout plan</h1><p>Follow today’s recommendation or choose a different workout.</p></div><div className="workout-header-actions"><button type="button" className="secondary-button" onClick={() => setCustomizerOpen(true)}><CalendarDays size={17}/> Customize week</button><div className="view-toggle"><button type="button" className={view === 'program' ? 'active' : ''} onClick={() => setView('program')}><Dumbbell size={17} /> Plan</button><button type="button" className={view === 'history' ? 'active' : ''} onClick={() => setView('history')}><CalendarDays size={17} /> Calendar</button></div></div></header>
    <div className="workout-safety"><ShieldCheck size={18} /><p><strong>Safety</strong> Stop for sharp pain, dizziness, or instability. Regress the movement and seek professional assessment when appropriate.</p></div>
    {view === 'program' ? <>
      ...
      <section className="day-tabs premium-day-tabs adaptive-day-tabs" aria-label="Adaptive weekly training planner">{week.days.map((planDay, index) => {
        ...
        const stateLabel = completed ? 'Completed' : plan.status === 'skipped' ? 'Skipped' : planDay.status === 'missed' ? 'Missed' : plan.selectedSessionTemplateId === 'full_rest' ? 'Rest day' : selectionSourceLabel(plan.selectionSource);
      ...
      {selectedPlan ? <section className="daily-recommendation card"><div><p className="eyebrow">{selectedSourceLabel}</p><h2>{sessionOption(controller.data.program, selectedTemplateId).name}</h2><p>{selectedPlan.recommendationReason}</p>{selectedPlan.selectedSessionTemplateId !== selectedPlan.recommendedSessionTemplateId ? <span><Pencil size={14}/> Your choice · App recommendation: {sessionOption(controller.data.program, selectedPlan.recommendedSessionTemplateId).name}</span> : <span><Sparkles size={14}/> {selectedSourceLabel} · based on completed workouts, not missed plans</span>}</div><div><span className={`recovery-pill ${recoveryForSession(controller.data, selectedTemplateId, selectedSnapshot.date).indicator.toLowerCase().replace(/\s+/g, '-')}`}>{recoveryForSession(controller.data, selectedTemplateId, selectedSnapshot.date).indicator}</span>{selectedSnapshot.date === today && selectedPlan.status !== 'completed' && isStrengthTemplate(selectedTemplateId) ? <><button type="button" className="primary-button" onClick={() => requestStart(selectedTemplateId, selectedSnapshot.date)}><Flame size={17}/> Start workout</button><button type="button" className="secondary-button" onClick={() => requestStart(selectedTemplateId, selectedSnapshot.date, 'short')}><Clock3 size={17}/> 30-minute version</button></> : null}{selectedSnapshot.date >= today && selectedPlan.status !== 'completed' ? <button type="button" className="secondary-button" onClick={() => openChooser(selectedSnapshot.date)}><Shuffle size={17}/> Change workout</button> : null}{selectedSnapshot.date >= today && selectedPlan.status !== 'completed' && selectedPlan.status !== 'skipped' ? <button type="button" className="text-button" onClick={() => controller.skipTrainingDay(selectedSnapshot.date)}>Mark as skipped</button> : null}</div></section> : null}
      <section className={`program-layout ${day.isRestDay ? 'rest-layout' : ''}`}>
        {day.isRestDay ? <article className="program-main recovery-main card"><header><div><p className="eyebrow">{selectedWeekday}’s recovery</p><h2>{displayWorkoutTitle(day)}</h2><span><HeartPulse size={16} /> {day.duration}</span></div><span className="recovery-badge">Recovery</span></header>{selectedTemplateId === 'cardio_recovery' ? <CardioLogger controller={controller} day={day} date={selectedSnapshot.date} /> : null}<div className="recovery-plan"><span className="recovery-orb"><HeartPulse size={26} /></span><div><h3>{selectedTemplateId === 'full_rest' ? 'Rest day' : 'Light recovery'}</h3><p>Use this plan for easy activity or rest. Planned recovery does not count as completed training.</p></div><ul>{day.recovery?.map((item) => <li key={item}><Check size={16} />{item}</li>)}</ul></div>{nextTrainingDay?.day ? <button type="button" className="secondary-button next-session" onClick={() => setSelectedDayId(nextTrainingDay.day!.id)}><CalendarDays size={18} /> Next strength workout: {displayWorkoutTitle(nextTrainingDay.day)}<ChevronRight size={17} /></button> : null}</article> : <article className="program-main premium-program-main card"><header><div><p className="eyebrow">{selectedWeekday}’s workout</p><h2>{displayWorkoutTitle(day)}</h2><span><Clock3 size={16} /> {day.focus} · {day.duration}</span></div>{editableTemplate ? <button type="button" className="secondary-button" onClick={() => setEditingDay(editableTemplate)}><Pencil size={16} /> Edit workout</button> : null}</header><div className="session-summary"><div><ToneIcon Icon={Dumbbell} tone="coral" /><span><strong>{day.exercises.length}</strong> exercises</span></div><div><ToneIcon Icon={ListChecks} tone="amber" /><span><strong>{day.exercises.reduce((sum, item) => sum + item.sets, 0)}</strong> working sets</span></div><div><ToneIcon Icon={Clock3} tone="cyan" /><span><strong>{day.duration}</strong> estimated</span></div><div className="session-muscles"><span>Main focus</span><strong>{[...new Set(day.exercises.flatMap((item) => exerciseMap.get(item.exerciseId)?.primaryMuscles ?? []))].slice(0, 4).join(' · ')}</strong></div></div><SessionMuscleOverview day={day} /><div className="program-list premium-program-list">{day.exercises.map((item, index) => { const exercise = exerciseMap.get(item.exerciseId)!; const previous = previousSets(previousCompleted, item.exerciseId); const best = previous.reduce((max, set) => Math.max(max, set.weightKg), 0); const recommendation = controller.data.progressionPlans.find((plan) => plan.exerciseId === item.exerciseId); const compound = exercise.movementPatterns.some((pattern) => ['Horizontal push', 'Horizontal pull', 'Vertical push', 'Squat', 'Hip hinge', 'Single-leg'].includes(pattern)); return <div className={`program-exercise ${compound ? 'compound' : 'accessory'}`} key={`${item.exerciseId}-${index}`}><span className="exercise-index">{String(index + 1).padStart(2, '0')}</span><button type="button" className="program-exercise__main" onClick={() => setDetails(exercise)} aria-label={`Open ${exercise.name} muscle and technique details`}><AnatomicalMuscleMap map={exercise.muscleMap} size="preview" /><div><h3>{exercise.name}{recommendation ? <em>Progression ready</em> : null}</h3><p>{muscleList(exercise.muscleMap.primary)}{item.notes ? ` · ${item.notes}` : ''}</p></div><div className="prescription"><strong>{item.sets} × {item.repMin}–{item.repMax}</strong><span>{item.warmupSets ? `${item.warmupSets} warm-up · ` : ''}{formatDuration(item.restSeconds)} rest · {item.rir} RIR</span></div>{best ? <div className="last-load"><strong>{best} kg</strong><span>previous best</span></div> : <div className="last-load"><strong>—</strong><span>no history</span></div>}<ChevronRight size={19} /></button></div>; })}</div>{selectedSnapshot.date === today ? <button type="button" className="primary-button start-workout" onClick={() => requestStart(selectedTemplateId, selectedSnapshot.date)}><Flame size={19} /> Start workout</button> : <div className="future-session-note"><CalendarDays size={17}/><p><strong>{selectedSnapshot.date > today ? 'Saved for this day' : 'Past planned workout'}</strong>{selectedSnapshot.date > today ? 'Open it on the day to start. You can change it at any time.' : 'Past plans are not carried forward.'}</p></div>}</article>}
        <WorkoutGuidance restDay={day.isRestDay} />
      </section>
    ...
    <Modal open={Boolean(replanReview)} onClose={() => setReplanReview(null)} title="Review updated recommendations" subtitle="Only recommended future workouts were recalculated. Workouts you selected were not changed.">{replanReview ? <div className="replan-review"><p>{replanChanges.length ? 'These future workout recommendations changed based on recovery and weekly targets:' : 'No future workout recommendations changed.'}</p>{replanChanges.length ? <div>{replanChanges.map((change) => <span key={change.date}><strong>{prettyDate(change.date)}</strong><small>{sessionOption(controller.data.program, change.before).name}</small><ChevronRight size={15}/><b>{sessionOption(controller.data.program, change.after).name}</b></span>)}</div> : null}<p className="replan-note">You can apply these recommendations or keep the current week.</p><footer><button type="button" className="primary-button" onClick={() => setReplanReview(null)}><Check size={17}/> Apply recommendations</button><button type="button" className="secondary-button" onClick={() => { controller.keepCurrentTrainingWeek(replanReview.previousPlans, replanReview.date); setReplanReview(null); }}><CalendarDays size={17}/> Keep current week</button><button type="button" className="text-button" onClick={() => { setReplanReview(null); setCustomizerOpen(true); }}>Review each day</button></footer></div> : null}</Modal>
    <Modal open={Boolean(startIntent)} onClose={() => setStartIntent(null)} title="Check before starting" subtitle="Review the recovery warnings and choose how to continue.">{startIntent ? <div className="start-warning-dialog">{selectedWarnings.map((warning) => <div className={`planner-warning ${warning.tone}`} key={warning.code}><AlertTriangle size={18}/><p><strong>{warning.title}</strong>{warning.message}</p></div>)}<div className="start-warning-actions">{activeSession ? <button type="button" className="secondary-button" onClick={() => { setStartIntent(null); setHideActiveSession(false); }}><Play size={17}/> Resume unfinished workout</button> : null}{activeSession ? <button type="button" className="secondary-button" onClick={() => { const intent = startIntent; controller.discardWorkout(activeSession.id); setStartIntent(null); launchStart(intent.templateId, intent.date, intent.mode); }}><X size={17}/> Discard unfinished workout and start</button> : null}{duplicateSession ? <button type="button" className="secondary-button" onClick={() => { setHistorySession(duplicateSession); setStartIntent(null); setView('history'); }}><History size={17}/> View today’s completed workout</button> : null}{selectedPlan && startIntent.templateId !== selectedPlan.recommendedSessionTemplateId ? <button type="button" className="secondary-button" onClick={() => { controller.selectTrainingSession(startIntent.date, selectedPlan.recommendedSessionTemplateId, 'adaptive_recommendation'); setStartIntent(null); }}><Sparkles size={17}/> Select recommended workout</button> : null}<button type="button" className="primary-button" onClick={continueStart}><Check size={17}/> Start {startIntent.mode === 'short' ? '30-minute version' : 'anyway'}</button><button type="button" className="text-button" onClick={() => { setStartIntent(null); openChooser(selectedSnapshot.date); }}>Choose another workout</button></div></div> : null}</Modal>
