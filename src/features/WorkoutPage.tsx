...
function ProgramEditor({ day, program, profile, onSave, onClose }: { day: WorkoutDay; program: WorkoutDay[]; profile: UserProfile; onSave: (day: WorkoutDay) => void; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title={`Edit ${displayWorkoutTitle(day)}`} subtitle="Changes are saved locally. Warnings guide you but never block deliberate choices." size="large" footer={<><button type="button" className="text-button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit" form="program-editor-form"><Save size={18} /> Save program</button></>}>
...
function SessionChooser({ controller, date, plan, onClose, onSelected }: { controller: AppController; date: string; plan: TrainingDayPlan; onClose: () => void; onSelected: () => void }) {
  ...
  return <Modal open onClose={onClose} title="Choose this day’s session" subtitle={`${prettyDate(date, true)} · the recommendation guides you, but never locks the choice.`} size="large">
