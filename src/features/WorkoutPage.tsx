...
import { Activity, AlertTriangle, ArrowLeft, BadgeCheck, BarChart3, BedDouble, CalendarDays, Check, CheckCircle2, ChevronRight, Circle, Clock3, Dumbbell, ExternalLink, Flame, Footprints, GripVertical, HeartPulse, History, Info, ListChecks, Minus, Pause, Pencil, Play, Plus, RotateCcw, Save, ShieldCheck, SkipForward, Sparkles, Trophy, X } from 'lucide-react';
...
function ExerciseDetail({ exercise, onClose }: { exercise: Exercise; onClose: () => void }) {
  return <Modal open onClose={onClose} title={exercise.name} subtitle={`${exercise.primaryMuscles.join(' · ')} — technique and safety`} wide>
    <div className="exercise-detail">
      {exercise.videoId ? <div className="demo-panel">
      ...
      </div> : <div className="video-fallback"><Play size={24} /><div><strong>No reliable embed selected</strong><p>{exercise.videoFallback}</p></div></div>}
      ...
      <div className="alternatives"><span>Equipment-compatible alternatives</span>{exercise.alternatives.map((item) => <strong key={item}>{item}</strong>)}</div>
    </div>
  </Modal>;
}
...
function ProgramEditor({ day, program, profile, onSave, onClose }: { day: WorkoutDay; program: WorkoutDay[]; profile: UserProfile; onSave: (day: WorkoutDay) => void; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title={`Edit ${day.title}`} subtitle="Changes are saved locally. Warnings guide you but never block deliberate choices." wide>
...
const templateOptions: Array<{ id: TrainingTemplate; label: string; detail: string }> = [
  { id: 'two-day-full-body', label: '2 days', detail: 'Full body A/B' },
  { id: 'three-day-full-body', label: '3 days', detail: 'Alternating A/B' },
  { id: 'four-day-upper-lower', label: '4 days', detail: 'Upper/lower A/B' },
];
...
export function WorkoutPage({ controller, activeSessionId, setActiveSessionId, onStartWorkout }: WorkoutPageProps) {
  ...
  return <div className="page workout-page">
    ...
    {view === 'program' ? <>
      ...
      <section className="day-tabs" aria-label="Weekly workout schedule">{controller.data.program.map((item) => { const completed = !item.isRestDay && controller.data.sessions.some((session) => session.dayId === item.id && session.completedAt && session.date === today); const isSelected = day.id === item.id; return <button type="button" key={item.id} aria-current={isSelected ? 'date' : undefined} className={`${isSelected ? 'active' : ''} ${item.isRestDay ? 'rest-day' : 'training-day'}`} onClick={() => setSelectedDayId(item.id)}><span>{item.label}</span><strong>{item.title}</strong><small>{completed ? <><BadgeCheck size={14} /> Completed</> : item.isRestDay ? 'Cardio / recovery' : isSelected ? 'Selected session' : 'Strength day'}</small></button>; })}</section>
      <WeeklyOverview controller={controller} />
      <section className={`program-layout ${day.isRestDay ? 'rest-layout' : ''}`}>
        {day.isRestDay ? <article className="program-main recovery-main card"><header><div><p className="eyebrow">{day.label} conditioning</p><h2>{day.title}</h2><span><HeartPulse size={16} /> {day.duration}</span></div><span className="recovery-badge">Non-lifting day</span></header><CardioLogger controller={controller} day={day} /><div className="recovery-plan"><span className="recovery-orb"><HeartPulse size={26} /></span><div><p className="eyebrow">Recovery guidance</p><h3>Build fitness without stealing from strength.</h3><p>Moderate cardio supports the plan. Keep the dose manageable and let lower-body recovery guide the pace.</p></div><ul>{day.recovery?.map((item) => <li key={item}><Check size={16} />{item}</li>)}</ul></div>{nextTrainingDay ? <button type="button" className="secondary-button next-session" onClick={() => setSelectedDayId(nextTrainingDay.id)}><CalendarDays size={18} /> Next: {nextTrainingDay.label} · {nextTrainingDay.title}<ChevronRight size={17} /></button> : null}</article> : <article className="program-main card"><header><div><p className="eyebrow">{day.label} session</p><h2>{day.title}</h2><span><Clock3 size={16} /> {day.focus} · {day.duration}</span></div><button type="button" className="secondary-button" onClick={() => setEditingDay(day)}><Pencil size={16} /> Edit program</button></header><div className="program-list">{day.exercises.map((item, index) => { const exercise = exerciseMap.get(item.exerciseId)!; const previous = previousSets(previousCompleted, item.exerciseId); const best = previous.reduce((max, set) => Math.max(max, set.weightKg), 0); return <div className="program-exercise" key={`${item.exerciseId}-${index}`}><span className="exercise-index">{String(index + 1).padStart(2, '0')}</span><button type="button" className="program-exercise__main" onClick={() => setDetails(exercise)}><div><h3>{exercise.name}</h3><p>{exercise.primaryMuscles.join(' · ')}{item.notes ? ` · ${item.notes}` : ''}</p></div><div className="prescription"><strong>{item.sets} × {item.repMin}–{item.repMax}</strong><span>{item.warmupSets ? `${item.warmupSets} warm-up · ` : ''}{formatDuration(item.restSeconds)} rest · {item.rir} RIR</span></div>{best ? <div className="last-load"><strong>{best} kg</strong><span>recent best</span></div> : <div className="last-load"><strong>—</strong><span>no history</span></div>}<ChevronRight size={19} /></button></div>; })}</div><button type="button" className="primary-button start-workout" onClick={() => activeSession && !activeSession.completedAt ? setHideActiveSession(false) : onStartWorkout(day.id)}><Flame size={19} /> {activeSession && !activeSession.completedAt ? 'Resume active workout' : `Start ${day.title}`}</button></article>}
