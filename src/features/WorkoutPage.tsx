  ...
  return <Modal open onClose={onClose} title={exercise.name} subtitle="Muscles, technique and safety" size="large">
    <div className="exercise-detail">
      <div className="exercise-detail-hero"><div>{exercise.videoId ? <div className="demo-panel">
        ...
        </div> : <div className="video-fallback"><Play size={24} /><div><strong>No reliable embed selected</strong><p>{exercise.videoFallback}</p></div></div>}</div><aside className="exercise-anatomy-detail"><div><p className="eyebrow">Anatomical muscle map</p><h3>Front and back</h3></div><AnatomicalMuscleMap map={map} view="both" size="detail" interactive /><MuscleMapLegend /></aside></div>
  ...
  return <div className="page workout-page">
    ...
    {view === 'program' ? <>
      ...
      <section className="day-tabs premium-day-tabs adaptive-day-tabs" aria-label="Adaptive weekly training planner">{week.days.map((planDay, index) => {
        ...
        const stateLabel = completed ? 'Completed' : plan.status === 'skipped' ? 'Skipped' : planDay.status === 'missed' ? 'Missed' : changed || plan.selectionSource.startsWith('user_') ? 'Selected' : isToday ? 'Recommended' : plan.selectedSessionTemplateId === 'full_rest' ? 'Rest day' : 'Upcoming';
      ...
      {selectedPlan ? <section className="daily-recommendation card"><div><p className="eyebrow">{selectedPlan.selectionSource.startsWith('user_') ? 'Selected workout' : 'Recommended workout'}</p><h2>{sessionOption(controller.data.program, selectedTemplateId).name}</h2><p>{selectedPlan.recommendationReason}</p>{selectedPlan.selectedSessionTemplateId !== selectedPlan.recommendedSessionTemplateId ? <span><Pencil size={14}/> Selected · Recommended: {sessionOption(controller.data.program, selectedPlan.recommendedSessionTemplateId).name}</span> : <span><Sparkles size={14}/> Based on completed workouts</span>}</div><div><span className={`recovery-pill ${recoveryForSession(controller.data, selectedTemplateId, selectedSnapshot.date).indicator.toLowerCase().replace(/\s+/g, '-')}`}>{recoveryForSession(controller.data, selectedTemplateId, selectedSnapshot.date).indicator}</span>{selectedSnapshot.date >= today && selectedPlan.status !== 'completed' ? <button type="button" className="secondary-button" onClick={() => openChooser(selectedSnapshot.date)}><Shuffle size={17}/> Change workout</button> : null}{selectedSnapshot.date >= today && selectedPlan.status !== 'completed' && selectedPlan.status !== 'skipped' ? <button type="button" className="text-button" onClick={() => controller.skipTrainingDay(selectedSnapshot.date)}>Mark as skipped</button> : null}</div></section> : null}
