...
export function ProfilePage({ controller, onDirtyChange }: ProfilePageProps) {
  ...
  return <div className="page profile-page">
    ...
    <section className="settings-layout">
      ...
      <aside className="settings-aside"><article className="profile-summary"><div className="profile-avatar">{draft.name.charAt(0).toUpperCase()}</div><p className="eyebrow">Your goal</p><h2>{draft.goalWeightKg} kg while maintaining muscle</h2><div><span><strong>{canonicalCurrent == null ? '—' : canonicalCurrent.toFixed(1)}</strong> kg now</span><ChevronRight size={18} /><span><strong>{draft.goalWeightKg}</strong> kg goal</span></div><section className="profile-plan-status"><span><Target size={15} /><strong>Week {planWeek}</strong><small>current week</small></span><span><Dumbbell size={15} /><strong>{trainingStreak}</strong><small>week streak</small></span><span><Activity size={15} /><strong>{consistency.percent}%</strong><small>consistency</small></span></section></article><article className="card data-card"><Settings2 size={20} /><h3>Local data</h3><p>Your profile, meals, measurements and workouts are stored locally in this browser.</p></article><details className="advanced-tools"><summary>Advanced data tools</summary><p>Restoring sample data replaces all local records and settings.</p><button type="button" className="reset-button" onClick={() => setResetOpen(true)}><RefreshCcw size={17} /> Restore sample data</button></details></aside>
    </section>
