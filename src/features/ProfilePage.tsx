...
interface ProfilePageProps { controller: AppController; }
...
export function ProfilePage({ controller }: ProfilePageProps) {
  ...
  return <div className="page profile-page">
    <header className="page-header"><div><p className="eyebrow">Profile & settings</p><h1>Your plan, editable.</h1><p>Manual targets always remain under your control.</p></div><button className="primary-button" type="button" disabled={!valid} onClick={() => { controller.updateProfile(draft); setSaved(true); window.setTimeout(() => setSaved(false), 1800); }}>{saved ? <Check size={18} /> : <Save size={18} />}{saved ? 'Saved' : 'Save changes'}</button></header>
