...
import { datesInWeek, personalRecordEvents, weeklyConsistency, weeklyVolumeSeries } from '../lib/engagement';
...
export function ProgressPage({ controller }: ProgressPageProps) {
  ...
  const nutritionSeries = Array.from({ length: 14 }, (_, index) => {
    ...
    return { date: date.slice(5), protein: Math.min(120, Math.round(totals.protein / Math.max(1, data.profile.proteinTarget) * 100)), calories: totals.calories ? Math.min(120, Math.round(totals.calories / data.profile.calorieTarget * 100)) : 0 };
  });
  ...
  return <div className="page progress-page premium-progress">
    ...
    <section className="progress-metrics premium-metrics">
    ...
    <section className="progress-layout premium-progress-layout">
      <article className="card weight-chart-card"><div className="section-title"><div><p className="eyebrow">Body weight</p><h2>Daily readings + rolling trend</h2></div><div className="range-toggle" aria-label="Weight chart range">{([['4w', '4 weeks'], ['3m', '3 months'], ['all', 'All time']] as const).map(([id, label]) => <button type="button" key={id} className={range === id ? 'active' : ''} onClick={() => setRange(id)}>{label}</button>)}</div></div>
        {chartData.length ? <div className="chart-area"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ left: -12, right: 8, top: 10, bottom: 0 }}><defs><linearGradient id="weightGradientPremium" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#5caef6" stopOpacity={.38} /><stop offset="100%" stopColor="#5caef6" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="rgba(139,161,171,.14)" vertical={false} /><XAxis dataKey="label" stroke="#778991" tickLine={false} axisLine={false} /><YAxis domain={['dataMin - 0.5', 'dataMax + 0.5']} stroke="#778991" tickLine={false} axisLine={false} width={48} /><Tooltip contentStyle={CHART_TOOLTIP_STYLE} labelFormatter={(_, payload) => payload?.[0]?.payload?.date ? prettyDate(payload[0].payload.date) : ''} /><Area type="monotone" dataKey="average" name="7-day average" stroke="#5caef6" strokeWidth={3} fill="url(#weightGradientPremium)" animationDuration={500} /><Line type="monotone" dataKey="weight" name="Daily measurement" stroke="#84969d" strokeWidth={1.4} dot={{ r: 2.5, fill: '#101a20' }} /></AreaChart></ResponsiveContainer></div> : <div className="empty-state"><Scale size={30} /><h3>No measurements in this range</h3><p>Log a weight or choose a longer time range.</p></div>}
      ...
      <article className="card consistency-card"><div className="section-title"><div><p className="eyebrow">Last 28 days</p><h2>Consistency calendar</h2></div><span>{consistency.percent}% this week</span></div><div className="consistency-calendar">{consistencyDays.map((item) => <div key={item.date} className={`score-${item.score}`} title={`${prettyDate(item.date)} · ${item.workout ? 'Workout ' : ''}${item.nutrition ? 'Nutrition ' : ''}${item.cardio ? 'Cardio' : ''}`}><span>{new Date(`${item.date}T12:00:00`).getDate()}</span><i>{item.workout ? <Dumbbell size={10} /> : item.cardio ? <Activity size={10} /> : item.nutrition ? <Check size={10} /> : null}</i></div>)}</div><div className="consistency-key"><span><i className="low" /> One signal</span><span><i className="mid" /> Two signals</span><span><i className="high" /> Three signals</span></div></article>
    </section>

    <section className="trend-grid">
      <article className="card trend-chart"><div className="section-title"><div><p className="eyebrow">Strength work</p><h2>Weekly training volume</h2></div><ToneIcon Icon={Dumbbell} tone="coral" /></div><div className="small-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={volumeSeries}><CartesianGrid stroke="rgba(139,161,171,.12)" vertical={false} /><XAxis dataKey="week" stroke="#778991" tickLine={false} axisLine={false} /><YAxis stroke="#778991" tickLine={false} axisLine={false} width={42} /><Tooltip contentStyle={CHART_TOOLTIP_STYLE} /><Bar dataKey="volume" name="Working-set volume (kg)" fill="#f08a78" radius={[6, 6, 0, 0]} animationDuration={500} /></BarChart></ResponsiveContainer></div><p className="chart-note">Volume is the sum of completed working-set load × repetitions. Warm-ups are excluded.</p></article>
      <article className="card trend-chart"><div className="section-title"><div><p className="eyebrow">Conditioning</p><h2>Cardio minutes</h2></div><ToneIcon Icon={Activity} tone="cyan" /></div><div className="small-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={cardioSeries}><CartesianGrid stroke="rgba(139,161,171,.12)" vertical={false} /><XAxis dataKey="week" stroke="#778991" tickLine={false} axisLine={false} /><YAxis stroke="#778991" tickLine={false} axisLine={false} width={35} /><Tooltip contentStyle={CHART_TOOLTIP_STYLE} /><Bar dataKey="minutes" name="Cardio minutes" fill="#51c8d8" radius={[6, 6, 0, 0]} animationDuration={500} /></BarChart></ResponsiveContainer></div><p className="chart-note">Only cardio you logged is shown.</p></article>
      <article className="card trend-chart nutrition-trend"><div className="section-title"><div><p className="eyebrow">Nutrition adherence</p><h2>Target completion</h2></div><ToneIcon Icon={Target} tone="blue" /></div><div className="small-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={nutritionSeries}><CartesianGrid stroke="rgba(139,161,171,.12)" vertical={false} /><XAxis dataKey="date" stroke="#778991" tickLine={false} axisLine={false} /><YAxis domain={[0, 120]} stroke="#778991" tickLine={false} axisLine={false} width={35} /><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value) => `${value}%`} /><Line type="monotone" dataKey="protein" name="Protein target" stroke="#5caef6" strokeWidth={2.5} dot={false} /><Line type="monotone" dataKey="calories" name="Calorie target" stroke="#efb44c" strokeWidth={2.2} dot={false} /></LineChart></ResponsiveContainer></div><p className="chart-note">Percent of each daily target logged. Empty days remain at zero.</p></article>
    </section>

    <section className="lower-progress-grid premium-lower-grid">
      ...
      <article className="card record-timeline"><div className="section-title"><div><p className="eyebrow">Personal records</p><h2>Earned improvements</h2></div><ToneIcon Icon={Medal} tone="amber" /></div>{records.length ? <div>{records.slice(-5).reverse().map((record) => <article key={`${record.date}-${record.exerciseId}-${record.estimatedOneRepMax}`}><span><Trophy size={15} /></span><div><strong>{record.exerciseName}</strong><p>{record.weightKg} kg × {record.reps} · {prettyDate(record.date)}</p></div></article>)}</div> : <div className="empty-state compact"><Trophy size={26} /><h3>No personal records yet</h3><p>Records appear only when a completed set improves the previous performance estimate.</p></div>}</article>

      <article className="card milestone-card"><div className="section-title"><div><p className="eyebrow">Meaningful milestones</p><h2>Progress worth noticing</h2></div><ToneIcon Icon={Award} tone="lime" /></div><div>{milestones.map(({ label, reached, detail, Icon }) => <article className={reached ? 'reached' : ''} key={label}><span>{reached ? <Check size={16} /> : <Icon size={16} />}</span><div><strong>{label}</strong><small>{detail}</small></div></article>)}</div></article>

      <article className="card habits-card"><div className="section-title"><div><p className="eyebrow">Today’s support habits</p><h2>Keep the basics visible</h2></div><span>{Object.values(habit).filter((value) => value === true).length} / 3</span></div>{[{ key: 'water' as const, label: 'Water target', detail: 'Hydration supports performance', Icon: Droplets }, { key: 'walk' as const, label: 'Daily walk', detail: 'Low-fatigue activity', Icon: Footprints }, { key: 'sleep' as const, label: 'Sufficient sleep', detail: 'Recovery and appetite control', Icon: BedDouble }].map(({ key, label, detail, Icon }) => <button type="button" key={key} className={habit[key] ? 'done' : ''} onClick={() => updateHabit(today, { [key]: !habit[key] })}><span><Icon size={19} /></span><p><strong>{label}</strong><small>{detail}</small></p><i>{habit[key] ? <Check size={16} /> : null}</i></button>)}<p className="habit-note"><CircleAlert size={16} /> Habits support the plan; they are not a score of personal worth.</p></article>
    </section>

    <article className="card weekly-review premium-review"><div className="section-title"><div><p className="eyebrow">Weekly check-in</p><h2>What the records say</h2></div><span>Current week</span></div><div className="review-table"><div><span>Weight average</span><strong>{trend.currentAverage.toFixed(1)} kg</strong><small>vs {previousAvgWeight.toFixed(1)} kg</small></div><div><span>Average calories</span><strong>{Math.round(averageCalories).toLocaleString()} kcal</strong><small>{withinTarget} logged days near target</small></div><div><span>Average protein</span><strong>{Math.round(averageProtein)} g</strong><small>{proteinDays} days on target</small></div><div><span>Training</span><strong>{completedWorkouts} / {plannedWorkouts}</strong><small>planned sessions</small></div></div><div className="weekly-coach"><Sparkles size={21} /><p><strong>Coach conclusion</strong>{weeklyCoach}</p></div></article>
  </div>;
}
