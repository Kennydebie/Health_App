  ...
  return <div className="page progress-page">
    ...
    <section className="progress-layout">
      <article className="card weight-chart-card"><div className="section-title"><div><p className="eyebrow">Body weight</p><h2>Rolling average leads</h2></div><span className="pill success"><TrendingDown size={14} /> {Math.abs(weightChange).toFixed(2)} kg / week</span></div>
        {chartData.length ? <div className="chart-area"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ left: -12, right: 8, top: 10, bottom: 0 }}><defs><linearGradient id="weightGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#83bde5" stopOpacity={0.32} /><stop offset="100%" stopColor="#83bde5" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="#2b3b36" vertical={false} /><XAxis dataKey="date" stroke="#7e9188" tickLine={false} axisLine={false} /><YAxis domain={['dataMin - 0.5', 'dataMax + 0.5']} stroke="#7e9188" tickLine={false} axisLine={false} width={48} /><Tooltip contentStyle={{ background: '#131c1a', border: '1px solid #30413c', borderRadius: 12 }} /><Area type="monotone" dataKey="average" name="7-day avg" stroke="#83bde5" strokeWidth={3} fill="url(#weightGradient)" /><Line type="monotone" dataKey="weight" name="Daily" stroke="#70847b" strokeWidth={1.5} dot={{ r: 3, fill: '#0b1211' }} /></AreaChart></ResponsiveContainer></div> : <div className="empty-state"><Scale size={30} /><h3>No weight entries</h3><p>Log your first weight to start the trend.</p></div>}
    ...
    <section className="lower-progress-grid">
      <article className="card exercise-history"><div className="section-title"><div><p className="eyebrow">Exercise history</p><h2>Performance signal</h2></div><label><span className="sr-only">Exercise</span><select value={exerciseId} onChange={(event) => setExerciseId(event.target.value)}>{[...new Set(data.sessions.flatMap((session) => session.sets.map((set) => set.exerciseId)))].map((id) => <option key={id} value={id}>{exerciseMap.get(id)?.name}</option>)}</select></label></div>
        {exerciseHistory.length ? <><div className="small-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={exerciseHistory}><CartesianGrid stroke="#2b3b36" vertical={false} /><XAxis dataKey="date" stroke="#7e9188" tickLine={false} axisLine={false} /><YAxis stroke="#7e9188" tickLine={false} axisLine={false} width={35} /><Tooltip contentStyle={{ background: '#131c1a', border: '1px solid #30413c', borderRadius: 12 }} /><Line type="monotone" dataKey="weight" name="Best kg" stroke="#8fd7b5" strokeWidth={3} dot={{ r: 4, fill: '#8fd7b5' }} /></LineChart></ResponsiveContainer></div><div className="history-caption">Latest best: <strong>{exerciseHistory.at(-1)?.weight} kg × {exerciseHistory.at(-1)?.reps}</strong><span>Volume {Math.round(exerciseHistory.at(-1)?.volume ?? 0).toLocaleString()} kg</span></div></> : <div className="empty-state"><Dumbbell size={28} /><h3>No performance yet</h3><p>Complete this exercise in a workout to create a trend.</p></div>}
      </article>
