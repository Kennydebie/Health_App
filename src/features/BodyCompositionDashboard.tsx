  ...
  return <div className="body-dashboard">
    {!latest ? <><section className="card body-empty"><Scale size={32} /><h2>Body composition</h2><div className="empty-state compact"><h3>No body-composition reading yet</h3><p>Upload a FitDays screenshot to add body-fat and muscle estimates.</p></div></section><WeightProgressPlan controller={controller} /></> : <>
      ...
      <section className="body-kpi-grid" aria-label="Primary body composition KPIs">
        {PRIMARY_METRICS.map((item) => {
          ...
          const favorableDown = item === 'weightKg' || item === 'bodyFatPercent' || item === 'fatMassKg';
           const trendText = previousChange == null ? 'No comparison yet' : previousChange === 0 ? 'Stable' : previousChange < 0 === favorableDown ? 'Moving toward target' : 'Changed away from target';
      ...
      <section className="body-detail-grid">
        <article className="card fat-quality"><h2>Body composition changes</h2>{fatQuality ? <><div><span><small>Weight</small><strong>{changeLabel(fatQuality.weight, 'kg')}</strong></span><span><small>Fat mass</small><strong>{changeLabel(fatQuality.fat, 'kg')}</strong></span><span><small>Fat-free mass</small><strong>{changeLabel(fatQuality.lean, 'kg')}</strong></span></div><p><TrendingDown size={17}/>{fatQuality.status}</p></> : <div className="empty-state compact"><Gauge size={28}/><h3>Two measurements required</h3><p>Take another measurement under similar conditions to compare tissue estimates.</p></div>}</article>
        <article className="card secondary-estimates"><h2>FitDays estimates</h2><div>{SECONDARY_METRICS.filter((item) => valueFor(latest, item) != null).map((item) => <span key={item}><small>{BODY_METRIC_LABELS[item]}</small><strong>{round(valueFor(latest, item))} {BODY_METRIC_UNITS[item]}</strong></span>)}</div><p><Info size={16}/> These values are device estimates. “Visceral fat” is shown as a device index, not kilograms or percent.</p></article>
      </section>
    </>}

    <details className="card body-goals" open={!latest}>
      <summary><span><Target size={19}/><strong>Body goals</strong><small>Edit body targets without changing nutrition targets</small></span></summary>
    ...
    <section className="card measurement-history body-history"><div className="section-title"><div><h2>Measurement history</h2></div><span>{measurements.length} {measurements.length === 1 ? 'entry' : 'entries'}</span></div>{measurements.length ? <div className="measurement-history-list">{[...measurements].reverse().map((measurement) => <article key={measurement.id}><span className={`measurement-source ${measurement.source === 'manual' ? 'manual' : ''}`}>{measurement.source === 'manual' ? <Scale size={15}/> : <Activity size={15}/>}</span><div><strong>{formatMeasurementDate(measurement.measuredAt)}</strong><small>{measurement.source === 'manual' ? 'Manual entry' : 'FitDays screenshot'} · saved {formatMeasurementDate(measurement.createdAt, false)}</small>{measurement.excludedFromTrend?.length ? <span className="outlier-note"><AlertTriangle size={13}/> Unusual change excluded from {measurement.excludedFromTrend.map((item) => BODY_METRIC_LABELS[item]).join(', ')} trend <button type="button" onClick={() => measurement.excludedFromTrend?.forEach((item) => confirmMeasurementMetric(measurement.id, item))}>Confirm & include</button></span> : null}</div><dl><div><dt>Weight</dt><dd>{round(measurement.weightKg)} kg</dd></div><div><dt>Body fat</dt><dd>{round(measurement.bodyFatPercent)}%</dd></div><div><dt>Muscle</dt><dd>{round(measurement.muscleMassKg)} kg</dd></div><div><dt>Waist</dt><dd>{round(measurement.waistCircumferenceCm)} cm</dd></div></dl></article>)}</div> : <div className="empty-state compact"><Scale size={28}/><h3>No body measurements yet</h3><p>Upload a FitDays screenshot or enter your weight to start tracking progress.</p></div>}</section>
